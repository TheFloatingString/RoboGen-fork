...
```""".format(object_category, "".join(semantics))
    ...
    elif model_provider == "openrouter/moonshotai/kimi-k2.5/baseten":
        actual_model = "moonshotai/kimi-k2.5/baseten"
    elif model_provider == "openrouter/moonshotai/kimi-k2.5/fireworks":
        actual_model = "moonshotai/kimi-k2.5/fireworks"
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task_description", type=str, default="put a pen into the box"
    )
    ...
    meta_path = "generated_task_from_description"
    assert args.object in partnet_mobility_dict.keys(), (
        "You should use articulated objects in the PartNet Mobility dataset."
    )
    if args.object_path is None:
        possible_object_ids = partnet_mobility_dict[args.object]
        args.object_path = possible_object_ids[
            np.random.randint(len(possible_object_ids))
        ]
    config_path = generate_from_task_name(
